...
Поскольку ссылка на инструкцию в домашнем задании ссылается на несуществующую страницу, воспользуемся даннлй [ссылкой](https://microk8s.io/docs/how-to-nfs)
...
chmod 0777 /srv/nfs
```
Редактируем etc/exports файл, чтобы разрешить подключение всех IP-адресов в 10.0.0.0/24 подсети
```
mv /etc/exports /etc/exports.bak
echo '/srv/nfs 10.0.0.0/24(rw,sync,no_subtree_check)' | sudo tee /etc/exports
```
Перезапускаем сервер и смотрим его статус
```
systemctl restart nfs-kernel-server
systemctl status nfs-kernel-server
```

![Alt_text](https://github.com/LeonidKhoroshev/kuber-homeworks/blob/main/2.2/screenshots/k8s6.png)
